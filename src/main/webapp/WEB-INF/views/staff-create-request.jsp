...
<title>Insert title here</title>
	...
	function addRow(tableID) {
 	...
 	function changeItemDDL()
	{
		var e1 = document.getElementById("catDDL");
		var strUser = e1.options[e1.selectedIndex].value;
		alert(strUser);
		
		var e2= document.getElementById("myItemSelect");
		...
		<div class="table-responsive">							
			<table class="table table-hover " id="dataTable">
				...
				<tr class="listBody">
						<td>1 </td>					     
						<td>					
						   	<select id="catDDL" onchange="changeItemDDL()">													
									<option>Select--- </option> 
									<c:forEach items="${categoryList}" var="categoryList">								
										<option value="${categoryList.categoryId}">${categoryList.categoryId} </option>
									</c:forEach>							
						...
						<td>
								...
								<select size="5" id="myItemSelectH">				
										<c:forEach items="${itemList}" var="itemList">	
											<c:choose>	
											<c:when test="${ itemList.category ==  '1001'}">												
													<option value="${itemList.category}" id="itemName">${itemList.category}*${itemList.itemName}</option>								
											</c:when>
											</c:choose>	
										</c:forEach>
